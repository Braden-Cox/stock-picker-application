...
def verify_pipeline_key(pipeline_key: str = Header(...)):
    if not secrets.compare_digest(pipeline_key, settings.PIPELINE_API_KEY):
...
def single_run(name):
    with _RUN_LOCKS_GUARD:
        lock = _RUN_LOCKS.setdefault(name, threading.Lock())
    if not lock.acquire(blocking=False):
        print(f"[{name}] already running, skipping this trigger.")
        yield False
        return
    try:
        yield True
...
def run_ticker_sync_background():
    ...
    with single_run("ticker_sync") as acquired:
        if not acquired:
            return
        db = next(get_db())
        try:
...
def run_ticker_status_background(min_posts, relevance_threshold):
    ...
    with single_run("ticker_status") as acquired:
        if not acquired:
            return
        db = next(get_db())
        try:
...
def get_active_tickers_for_scrape(db):
    from app.models.ticker import Ticker
    active = db.query(Ticker).filter(Ticker.is_active == True).all()
    return [
        {"ticker": t.ticker, "is_active": t.is_active, "needs_prefix": t.needs_prefix}
        for t in active
    ]
...
def run_scrape_pipeline_background(limit_tickers, limit_calls, all):
    ...
    with single_run("scrape") as acquired:
        if not acquired:
            return
        db = next(get_db())
        try:
            ...
            store_tickers(cleaned_tickers, db)
            scrape_list = get_active_tickers_for_scrape(db)
            args = {}
            ...
            if all:
                args['all'] = all
            posts = scrape_posts(scrape_list, **args)
...
def run_classification_pipeline_background(relevance_limit, sentiment_limit):
    ...
    with single_run("classification") as acquired:
        if not acquired:
            return
        db = next(get_db())
        try:
...
def run_verification_pipeline_background(user_id=None, skip_scrape=False, limit=None):
    from app.database import get_db
    from app.pipeline.verify_picks import run_verify_picks_pipeline, get_users_with_pending_posts

    with single_run("verification") as acquired:
        if not acquired:
            return
        db = next(get_db())
        try:
            if user_id:
                run_verify_picks_pipeline(user_id, db, limit=limit, skip_scrape=skip_scrape)
            else:
                users = get_users_with_pending_posts(db)
                for user in users:
                    try:
                        run_verify_picks_pipeline(user[0], db, limit=limit, skip_scrape=skip_scrape)
                    except Exception as e:
...
def run_ranking_pipeline_background(top_n=20):
    ...
    with single_run("ranking") as acquired:
        if not acquired:
            return
        db = next(get_db())
        try:
...
def trigger_verification(background_tasks: BackgroundTasks, user_id: str = None, skip_scrape: bool = False, limit: int = None):
    background_tasks.add_task(run_verification_pipeline_background, user_id=user_id, skip_scrape=skip_scrape, limit=limit)
